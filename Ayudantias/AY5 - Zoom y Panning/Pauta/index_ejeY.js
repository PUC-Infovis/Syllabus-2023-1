...
div_botones
	.append("button")
	.attr("id", "btn-top")
	.text("Zoom a la mitad de arriba")
...
const visualizacion = (data_dict) => {
	...
	const manejadorZoom = (evento) => {
		...
		// Actualizamos el rango de las escalas considerando la transformación realizada.
		// Usamos rescale, ya que las ventas son una escala contínua
		const reescalaVentas = transformacion.rescaleY(escalaVentas)
		const reescalaPosicionVentas = transformacion.rescaleY(escalaPosicionVentas)

		// Actualizamos posición en Y y alto de las barras considerando la nueva escala.
		contenedorBarras
			.selectAll("rect")
			.attr("y", (d) => reescalaPosicionVentas(d.max))
			.attr("height", d => reescalaVentas(d.max) - reescalaVentas(d.min))

		contenedorPuntosMax
			.selectAll("circle")
			.attr("cy", d => reescalaPosicionVentas(d.max));

		contenedorPuntosMin
			.selectAll("circle")
			.attr("cy", d => reescalaPosicionVentas(d.min))

		contenedorLineaMedian
			.selectAll("line")
			.attr("y1", d => reescalaPosicionVentas(d.median))
			.attr("y2", d => reescalaPosicionVentas(d.median))

		// Actualizamos el ejeY usando.scale para cambiarle la escala
		contenedorEjeVentas.call(ejeVentas.scale(reescalaVentas).ticks(5))
